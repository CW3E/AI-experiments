...
def chunked_prediction(
    ...
    verbose: bool = False,
    constraints: xarray.Dataset = None, 
    buffer: int = 10, 
    relaxation = 1.0, 
    num_steps_for_constraints = None, 
) -> xarray.Dataset:
  ...
  for prediction_chunk in chunked_prediction_generator(
      ...
      num_steps_for_constraints=num_steps_for_constraints,  
      ):  
...
def chunked_prediction_generator(
  ...
  num_target_steps = targets_template["time"].size
  ...
  if num_steps_for_constraints is None: 
    num_steps_for_constraints = num_chunks - 1
  ...
  for chunk_index in range(num_chunks):
    ...
    ##### constraints ################################################################
    if (constraints is not None) and (chunk_index < num_steps_for_constraints):
      varnames = predictions.data_vars.keys()
      constraint_weights = set_constraint_weights(constraints['10m_u_component_of_wind'][0,0,...], buffer)
      ...
      for varname in varnames:
        ...
        constraints_var = constraints[varname][:, [chunk_index]] 
        replaced = predictions[varname].loc[..., constraints_var.lat, constraints_var.lon].copy()
        replaced['time'] = constraints_var.time  # The time of predictions is always 21600000000000
        prescribed = constraints_var*constraint_weights + replaced*constraint_weights_inverse
        prescribed = permute_dims(prescribed, 0, 1)  # predictions: [time, batch, ...]; prescribed: [batch, time, ...]

        writable_data = np.full_like(predictions[varname].values, np.nan)
        writable_data[...] = predictions[varname].values 
        lon_indices = np.where(predictions.lon.isin(constraints.lon).values)[0]
        lat_indices = np.where(predictions.lat.isin(constraints.lat).values)[0]
        writable_data[..., lat_indices[0]:lat_indices[-1]+1, lon_indices[0]:lon_indices[-1]+1] = prescribed.values
        writable_data = xarray.DataArray(writable_data, dims=predictions[varname].dims, coords=predictions[varname].coords)
        predictions[varname] = writable_data    
    ######################################################################################################
...
def set_constraint_weights(constraint_weights_template:xarray.DataArray, buffer:int=5):
...
def permute_dims(arr:xarray.DataArray, d1, d2):
...
def _get_next_inputs(
  ...
  num_inputs = prev_inputs["time"].size 
  return (
      xarray.concat(
